	...
	<div id="wrapper">
			...
			<c:forEach items="${cartList}" var="dto" varStatus="status">
				<tr>
					<td style="text-align: center"><input type="checkbox"
						name="chk"></td>
					<c:forEach items="${productInfo}" var="dtoProduct"
						begin="${status.index}" end="${status.index}" varStatus="pStatus">
						<td style="border-right: none"><img
							src="http://192.168.0.80:8080/RAUM/resources/image/${dtoProduct.image}"
							width="60" height="60"></td>
						<td width="340" style="border-left: none"><a
							href="/ProjectShare/MainProductDetail.do?product_num=${dtoProduct.product_num}">${dtoProduct.name}</a>
							<br>${dto.size}</td>
					</c:forEach>
					<td style="text-align: center">${dto.qty}개</td>
					<td>
						<%-- 사용일 ${dto.startDate}~${dto.endDate}<br> --%>대여일
					...
					<c:forEach items="${productInfo}" var="dtoProduct"
						...
						<c:set var="sum"
							value="${sum + (dtoProduct.price + (dto.rentDay - 1) * 1000)*(dto.qty)}" />
					</c:forEach>
					<td style="text-align: center"><input type="button"
						value="바로대여" onClick="rentThisProduct(${dto.seq});"> <br>
					<input type="button" value="삭제" onClick="deleteCart(${dto.seq});"></td>
				</tr>
			</c:forEach>
			<tr style="text-align: right">
				<td colspan="8">총 대여금액 <c:out value="${sum}" />원 + 배송비 3,000원 =
					총 결제금액 : ${sum + 3000}원
				</td>
			</tr>
		</table>
		...
		<div style="float: right;">
			<input type="button" value="쇼핑계속하기"
				onClick="location.href='MainProductList.do'"> <input
				type="button" value="선택상품 대여" name="btnRentChecked"
				id="btnRentChecked" onClick="rentChecked();"> <input
				type="button" value="전체상품 대여" name="btnRentAll" onClick="rentAll();">
		</div>
		<br>
		<br>
		<br>
		<br> <img
			src="http://www.ddaengshop.com/common/img/tit_basket_take.gif"
			alt="장바구니" />
		...
		<table border="1" style="border-collapse: collapse;" width="100%">
			...
			<tbody>
				...
				<tr>
					...
					<td style="padding: 4px;">원하시는 제품 좌측에 체크박스 선택 후 하단에 [선택상품대여]
						버튼을 눌러 주세요.</td>
				</tr>
				<tr>
					<td style="padding: 4px;">결제방법 및 할인</td>
					<td style="padding: 4px;">신용카드, 실시간계좌이체, 무통장입금, 쿠폰할인, 보유 적립금
						할인</td>
				</tr>
				<tr>
					<td style="padding: 4px;">배송비</td>
					<td style="padding: 4px;">안전하고 빠른 우체국택배로 기본배송비는 3,000원으로 모든
						제품이 묶음 상품입니다.(묶음불가상품은 따로 표기 됩니다.)</td>
				</tr>
				<tr>
					<td style="padding: 4px;">계속쇼핑</td>
					<td style="padding: 4px;"><span class="fb">[쇼핑계속하기]</span> 버튼을
						누르면 상품 화면으로 돌아갑니다.</td>
				</tr>
			</tbody>
		</table>
		<br>
