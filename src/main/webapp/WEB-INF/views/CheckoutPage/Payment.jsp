...
<body>
	 ...
	 <form action="write.do" name="myform" id="myform" method="post" onsubmit="return formSubmit();">
			...
			<input type="hidden" name="Rent_seq" value="${rentSeq }">
			<table border="1" width="1000" height="20">
				...
				<c:forEach items="${Product}" var="dto">
					<tr>
						<td><img src="http://192.168.0.80:8080/RAUM/resources/image/${dto.image}" alt="${dto.image}" width="50" height="50"></td>
						<td>  ${dto.qty} </td>
				...
				<table border="2" width="1000" height="20">
					...
					<tbody>
						<tr class="selectbesong">
							<%-- <td>배송지 선택</td>
							<td>
							<input type="radio" name="addryn_0" id="addryn_02" onclick="r_null()" checked="checked" /><label for="addryn_02"> 새로운 주소 입력</label>
							<c:forEach items="${Address}" var="dto">
							<input type="radio" name="addryn_0" id="${dto.seq}" onclick="r_null()" /><label for="${dto.seq}"> ${dto.addressName}</label>
							</c:forEach>
							<!-- <input type="radio" name="addryn_0" id="addryn_01" value="no" onclick="r_fill()" /><label for="addryn_01"> 주문고객과동일 주소</label> -->
							</td>
						</tr>
						<tr> --%>
							<td>받으시는 분</td>
